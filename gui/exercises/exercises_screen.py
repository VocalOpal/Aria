...
from ..design_system import (
    ...
    PrimaryButton, SecondaryButton, InfoCard, TitleLabel,
    HeadingLabel, BodyLabel, CaptionLabel, create_gradient_background,
    add_card_shadow
)
from ..components.breathing_guide import BreathingGuideWidget
...
class ExerciseItemWidget(QWidget):
    ...
    def init_ui(self):
        ...
        header_layout.setSpacing(AriaSpacing.MD)

        # Title section
        title_section = QVBoxLayout()
        title_section.setSpacing(AriaSpacing.XS)

        # Exercise name
        title = QLabel(self.exercise_data['name'])
        title.setFont(QFont("Arial", AriaTypography.BODY, QFont.Weight.Bold))
...
class ActiveExerciseWidget(QWidget):
    ...
    def init_ui(self):
        ...
        main_layout.setSpacing(AriaSpacing.LG)
        ...
        """)
        add_card_shadow(content_frame)
        content_layout = QVBoxLayout(content_frame)
        content_layout.setSpacing(AriaSpacing.LG)

        # Exercise header section
        header_section = QVBoxLayout()
        header_section.setSpacing(AriaSpacing.SM)

        # Exercise title
        title = HeadingLabel(f"Active: {self.exercise_data['name']}")
        ...
        title.setStyleSheet(f"""
            ...
            font-weight: 700;
            background: transparent;
            margin-bottom: {AriaSpacing.SM}px;
        """)
        ...
        if self.exercise_data['breathing_focus']:
            ...
            # Add breathing guide widget
            self.breathing_guide = BreathingGuideWidget()
            breathing_section.addWidget(self.breathing_guide, alignment=Qt.AlignmentFlag.AlignCenter)
    ...
    def stop_exercise(self):
        ...
        try:
            if self.session_active:
                ...
                # Stop breathing guide if applicable
                if self.breathing_guide:
