...
class VoiceTrainer:
    ...
    def __init__(self, config_file="data/voice_config.json"):
        ...
        # Get current profile's config file
        if self.profile_manager.current_profile_id:
            profile_config = self.profile_manager.get_profile_config_path()
        else:
            profile_config = config_file
        
        self.factory = get_component_factory(profile_config)

        self.config_manager = VoiceConfigurationManager(profile_config)
        self.session_manager = VoiceSessionManager(profile_config)
        self.achievement_system = VoiceAchievementSystem()

        self.training_controller = VoiceTrainingController()
        self.audio_coordinator = VoiceAudioAnalyzerCoordinator(profile_config)
    ...
    def _initialize_dependencies(self):
        ...
        # New managers
        self.snapshot_manager = self.factory.get_component('snapshot_manager')
        self.health_analyzer = self.factory.get_component('health_analyzer')
        self.session_templates = self.factory.get_component('session_templates')
