...
class VoiceAchievementSystem:
    ...
    def export_for_discord(self) -> dict:
        """
        Export achievements for Discord bot integration
        
        Returns:
            Dict with achievement data ready for export
        """
        # This method allows the export_manager to call it
        # Returns empty dict for now - Discord bot will compute achievements from session data
        return {
            'achievements': {
                'note': 'Achievements computed from session data by Discord bot'
            }
        }
